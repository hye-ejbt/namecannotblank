...
export function isConnection(
  ...
  // 시작 노드 ID 를 함께 넘겨 양방향(꺾이는 부분 포함) walk 가능하게 함
  return checkSequentialNoBranching(
    pLinks,
    linksAssociatedWithNodes[0],
    node.getNodeId(),
    options
  );
}
 ...
 *
 *  시작 노드 → 루트 링크 → 다음 링크 → ... → 종료 링크 까지
 *  "현재 노드 ID" 를 들고 다니며 한 칸씩 전진.
 *
 *  ★ 양방향 (꺾이는 부분) 지원:
 *    링크는 stNdId/edNdId 어느 쪽으로도 진입 가능.
 *    - 진입 노드가 link.stNdId 면 다음 노드 = link.edNdId
 *    - 진입 노드가 link.edNdId 면 다음 노드 = link.stNdId
 *    원본 passNodeProcess 의 다음 코드와 동일한 패턴:
 *      tmpNodeId = outLinkInfo.stNdId === selectNodeInfo.id
 *        ? outLinkInfo.edNdId
 *        : outLinkInfo.stNdId;
 *
 *  매 단계마다 "현재 노드 에서 출발하는 다음 링크 후보 수" 를 검사:
 *    0개 → 종료 링크로 판단, walk 정상 종료
 *    1개 → 그 링크로 전진 + 노드 ID 갱신
 *    2개+ → 중간 분기 → invalid
 *
...
function checkSequentialNoBranching(
  ...
  // 시작 노드 ↔ 루트 링크 연결 방향 검증 + 다음 노드 결정
  if (currentLink.stNdId !== startNodeId && currentLink.edNdId !== startNodeId) {
    return {
      isConnected: false,
      errorMessage: '유효하지 않은 진출 링크가 있습니다',
      errorMessageDetail:
        `루트 링크(id=${currentLink.id}, stNdId=${currentLink.stNdId}, edNdId=${currentLink.edNdId}) 가 ` +
        `시작 노드(id=${startNodeId}) 와 직접 연결되지 않습니다.`,
      failedAt: { index: validLinks.indexOf(currentLink), fromLinkId: -1, toLinkId: currentLink.id, reason: 'no-root' },
    };
  }

  let currentNodeId: number =
    currentLink.stNdId === startNodeId ? currentLink.edNdId : currentLink.stNdId;

  // ─── 순차 walk — 매 단계마다 currentNodeId 기준 분기 검사 ─
  const visited = new Set<number>();
  visited.add(currentLink.id);

  while (true) {
    // currentNodeId 에 양방향으로 연결된 다음 링크 후보 (outLinkIds 내에서)
    const nextCandidates: LinkData[] = validLinks.filter(
      (l) =>
        l.id !== currentLink!.id &&
        !visited.has(l.id) &&
        (l.stNdId === currentNodeId || l.edNdId === currentNodeId)
    );
    ...
    if (nextCandidates.length >= 2) {
      return {
        ...
        errorMessage: '유효하지 않은 진출 링크가 있습니다',
        errorMessageDetail:
          `링크 ${currentLink.id} 의 끝 노드(${currentNodeId})에서 분기됩니다. ` +
    ...
    // 다음 링크로 전진하면서 currentNodeId 갱신 (꺾임 처리)
    currentNodeId =
      nextLink.stNdId === currentNodeId ? nextLink.edNdId : nextLink.stNdId;
